...
class StochEnv:
    ...
    def __init__(self, heston_params, spot_correlations, T, num_periods, vol_of_correlation=0.002):
        ...
        self.vol_of_correlation  = vol_of_correlation

    def simulate(self, num_sims):
        Sr = np.zeros((self.num_periods, num_sims, self.na))
        Vr = np.zeros((self.num_periods, num_sims, self.na))
        Cr = np.ones((self.num_periods, num_sims, self.na, self.na))

        X = np.zeros(self.na)
        V = deepcopy(self.heston_params[:, 0])
        U = np.repeat(self.U0[np.newaxis, :, :], num_sims, axis=0)
        rho = np.ones((num_sims, self.na, self.num_sim_steps_per_period))
        for i in range(self.na):
            rho[:, i, :] = self.heston_params[i, -1]
        rho_bar = (1 - rho**2)**0.5

        for i in range(self.num_periods):
            U = U + np.random.multivariate_normal(np.zeros(self.na), self.vol_of_correlation**2*np.eye(self.na), size=(num_sims, self.na))
            U = U / np.linalg.norm(U, axis=1).reshape((num_sims, 1, self.na))
            L = np.transpose(U, axes=[0,2,1])
            W = np.random.normal(size=(num_sims, self.na, self.num_sim_steps_per_period))
            W1 = np.matmul(L, W)
            W2 = rho * W1 + rho_bar * np.random.normal(size=(num_sims, self.na, self.num_sim_steps_per_period))
